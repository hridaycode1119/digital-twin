    ...
    <AnimatePresence>
      {organ && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            {/* Header */}
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-blue-50/50 via-white to-slate-50/50">
              <div className="flex items-center gap-3">
                <div className="p-3 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-blue-600 font-bold">
                  <Activity className="w-6 h-6 text-blue-600" />
                </div>
                <div>
                  <div className="flex items-center gap-2.5">
                    <h3 className="text-xl font-bold text-slate-900">{organ.name} System</h3>
                    <OrganBadge status={organ.status} />
                  </div>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Digital Twin Physiological Telemetry • Real-Time Health State
                  </p>
                </div>
              </div>
              <motion.button
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {organ.metrics.map((metric, idx) => (
                    ...
                    >
                      <p className="text-xs font-medium text-slate-500 truncate">{metric.name}</p>
                      ...
                      {metric.isAbnormal && (
                        <span className="mt-1 inline-flex items-center gap-1 text-[10px] font-semibold text-amber-700">
                          <AlertTriangle className="w-3 h-3" /> Needs Monitoring
                        </span>
                      )}
                    </motion.div>
                  ))}
              ...
              <div className="p-4 rounded-2xl bg-slate-50/70 border border-slate-200/60">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs font-semibold text-blue-600">Current: {organ.score}/100</span>
                </div>
                <div className="h-40 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={organ.historicalTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      <defs>
              ...
              <div className="p-4 rounded-2xl bg-blue-50/50 border border-blue-100">
                <h4 className="text-xs font-bold uppercase tracking-wider text-blue-800 flex items-center gap-1.5 mb-1.5">
                  <CheckCircle2 className="w-4 h-4 text-blue-600" />
                  AI Clinical Summary
                </h4>
                <p className="text-sm text-slate-700 leading-relaxed">{organ.clinicalInsights}</p>
              </div>
